#!/usr/bin/env bash
# 英文牙科 参数细扫 —— 学生 = Llama-3.3-70B（QLoRA 4bit），α=0，seed=42。
# 网格：rank{4,8,16} × lr{1e-4,2e-4,3e-4,5e-4} × epoch{1} = 12 组（epoch2/lr5e-4 在 Qwen2.5 上已验证掉点，跳过）。
...
DATA="data/en_dental"
SEED=42
RESULT="ablation/results_en_dental_llama70b.tsv"
...
for rank in "${RANKS[@]}"; do
  for lr in "${LRS[@]}"; do
    for ep in "${EPOCHS[@]}"; do
      ...
      out="ablation/runs/en_dental_llama70b/${name}"
      ...
      val_acc=$("$PY" shared/eval_choice_head.py --model "$STUDENT" --adapter "$out" \
                 --test "$DATA/val.jsonl" --lang en --quantize 4bit 2>/dev/null | grep -oE 'acc=[0-9.]+' | head -1 | cut -d= -f2)
      test_acc=$("$PY" shared/eval_choice_head.py --model "$STUDENT" --adapter "$out" \
                 --test "$DATA/test.jsonl" --lang en --quantize 4bit 2>/dev/null | grep -oE 'acc=[0-9.]+' | head -1 | cut -d= -f2)
