#!/usr/bin/env bash
# 英文牙科 v3 重训：放宽 R1（补口腔内科/唾液腺词）后回收 81 题，train 566->631。
# test 保持原 84 题不动，保证与历史结果可比。
set -uo pipefail
...
LOG="retrain_v3_queue.log"
exec > >(tee -a "$LOG") 2>&1
export DISTILL_PROMPT_LANG=en
export DISTILL_USE_CHAT_TEMPLATE=1

echo "[$(date '+%F %H:%M:%S')] retrain-v3 queue START"

echo "[$(date +%H:%M:%S)] TRAIN en_dental Qwen2.5-32B (train_v3 631)"
OUT="experiments/en_dental/runs/32B_v3_r8_lr3e4_s42"
mkdir -p "$OUT"
"$PY" shared/train_choice_head_distill.py \
  --model_name "$M32" --data_path data/en_dental/train_v3.jsonl \
...
echo "[$(date +%H:%M:%S)] TRAIN en_dental Llama-70B (train_v3 631, 4bit)"
OUT="experiments/en_dental/runs/Llama70B_v3_s42"
mkdir -p "$OUT"
"$PY" shared/train_choice_head_distill.py \
  --model_name "$M70" --data_path data/en_dental/train_v3.jsonl \
  --val_path data/en_dental/val.jsonl --output_dir "$OUT" \
  --num_epochs 1 --batch_size 1 --gradient_accumulation_steps 8 \
...
echo "[$(date '+%F %H:%M:%S')] retrain-v3 queue END"
