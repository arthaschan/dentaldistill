#!/usr/bin/env python3
"""英文 v2（148 题）逐题预测 + McNemar 配对检验（教师 Qwen3-32B vs 学生 Llama-70B 3-seed）。"""
import json
import os
import sys
...
TEST = os.path.join(ROOT, "data/en_dental_v2/test.jsonl")
...
def main():
    ...
    n = len(rows)
    gt = [str(r["Answer"]).strip().upper() for r in rows]
    ...
    t_acc = sum(t_pred)
    print(f"教师 Qwen3-32B: {100*t_acc/n:.2f}% ({t_acc}/{n})")

    # 学生 3 seed（Llama-70B 4bit）
    ...
    for seed in ["11", "42", "8"]:
        adapter = os.path.join(ROOT, f"experiments/en_dental/runs/Llama70B_v2_r16_lr1e-4_s{seed}")
